...
void Draw_Init(void)
{
	...
	// Draw the Boarder for "W55FA93 SpiWriter (..)"
	s_sRect.u32StartX =0;
	s_sRect.u32StartY = 0;
	...
	sprintf(Array1, "W55FA93 SpiWriter (v%d.%d)",MAJOR_VERSION_NUM, MINOR_VERSION_NUM);
#if 0
	Draw_Font(COLOR_RGB16_WHITE, &s_sDemo_Font,
						(_LCM_WIDTH_ - 32*g_Font_Step)/2, // 32 Character for "W55FA93 SpiWriter (%s"
						0,
						Array1);
#else
	Draw_Font(COLOR_RGB16_WHITE, &s_sDemo_Font,
						0, // 32 Character for "W55FA93 SpiWriter (%s"
						0,
						Array1);
#endif
